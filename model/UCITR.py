...
class UCITR(torch.nn.Module):
    def __init__(self, length, hidden_dim, input_dim, device,args):
        ...
        self.Wk = nn.ModuleList(
            [nn.Linear(hidden_dim, hidden_dim) for i in range(self.predict_timestep)])
        ...
        self.linear_encoder = nn.Linear(2*input_dim,hidden_dim)
        self.embedding_linear = nn.Linear(36+3*hidden_dim,hidden_dim)
        self.conv_encode1 = torch.nn.ConvTranspose1d(self.length, 1, kernel_size=5)
        self.conv_encode2 = torch.nn.ConvTranspose1d(self.length, 1, kernel_size=11)
        self.conv_encode3 = torch.nn.ConvTranspose1d(self.length,1,kernel_size=23)
    ...
    def forward(self, inputs, time_delat, missing_mask):
        ...
        pred = torch.empty((self.batchsize, self.predict_timestep, self.hidden_dim)).float().to(self.device)
        for i in range(self.predict_timestep):
            linear = self.Wk[i]
            pred[:,i:i+1] = linear(hidden[:,-self.predict_timestep:1-self.predict_timestep])
        return total_hidden,output,pred,hidden_back

    def inner_encode(self, inputs, time_delat, missing_mask):
        hidden_interval, (ih_T, ic_T) = self.encoder_interval(inputs, time_delat)
        ...
        hidden = torch.cat((hidden_interval, hidden_msak), dim=2)
        hidden = self.linear_encoder(hidden)
        hidden1 = self.conv_encode1(hidden)
        hidden2 = self.conv_encode2(hidden)
        hidden3 = self.conv_encode3(hidden)
