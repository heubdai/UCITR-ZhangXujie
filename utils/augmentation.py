...
def time_warp(x, sigma=0.2, knot=4):
    ...
    for i, pat in enumerate(x):
        for dim in range(x.shape[2]):
            ...
            ret[i,:,dim] = np.interp(np.clip(scale*time_warp, 0, x.shape[1]-1), orig_steps, pat[:,dim]).T
    return ret


def time_warp_mask(x, mask, sigma=0.1, knot=4):
    # sigma越小越接近，越大随机性越高 knot
    from scipy.interpolate import CubicSpline
    ...
    for i, pat in enumerate(x):
        for dim in range(x.shape[2]):
            orig_steps = np.squeeze(np.array(np.where(mask[i,:,dim] != 1)))
            time_warp = CubicSpline(warp_steps[:, dim], warp_steps[:, dim] * random_warps[i, :, dim])(orig_steps)
            # print("origen step",orig_steps,"time warp",time_warp)
            scale = (x.shape[1] - 1) / time_warp[-1]
            ret[i, :, dim] = np.interp(np.arange(pat.shape[0]), np.clip(scale * time_warp, 0, x.shape[1] - 1), pat[orig_steps,dim]).T
    return ret
...
def into_missing_value(x_size):
    return np.random.choice(2, size = x_size, replace = True, p = [0.8,0.2])
